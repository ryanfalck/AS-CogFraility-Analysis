---
title: "REVISED FINAL Action Seniors - CogFrailty Analyses (November 3, 2022)"
author: "RSF"
date: "2022-11-03"
output: 
  html_document:
...
# 6) Supplementary Material S2 - Differences in individual monthly compliance 
...
# 7) Supplementary Material S3 - Sex differences in monthly compliance
...
Estimated differences in average compliance for cognitively frail participants based on sex
```{r}
lsmeans(comply.2,~Sex)
contrast(lsmeans(comply.2, ~Sex), "pairwise", adj="none") #No significant difference overall
```

Estimated differences in average compliance for cogntiively frail participants based on sex by timepoint
```{r}
...
# 8) Table 2 - Primary and secondary outcomes at 12 months by cognitive frailty status and treatment group

Falls Incident Rate Ratio (Cognitively Frail Participants Only)
...
Differences in total exposure based on group (Cognitively Frail Participants Only)
...
# 9) Figure 2 - Accumulation of falls by treatment group for cognitively frail participants
...
# 10) Table 3 - Primary and secondary outcomes at 12 months in exercise group based on cognitive frailty status and compliance to the intervention 
...
```{r}
SPPB_lm<-lmer(SPPB~Compliant*factor(time) + scale(Gender,scale=FALSE) + SPPBbaseline + 1 + (1|ID), subset(OEP3, exclude == 0 & CogFrail =="Yes"))
anova(SPPB_lm)
